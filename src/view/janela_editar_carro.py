...
from controller.janela_editar_controller import JanelaEditarController 

class JanelaEditarCarro(Tk.Toplevel):
	def __init__(self, gerenciador, carro):
		super().__init__()
		self.gerenciador = gerenciador
		self.controller = JanelaEditarController(gerenciador, self)
		self.carro = carro
	...
	def __adicionarBotaoFinalizar(self):
		Tk.Button(self.container, command=self.controller.editar, text="Finalizar").grid(column=1, row=5, pady=(0,10))
